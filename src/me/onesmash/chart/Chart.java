...
import me.onesmash.splineinterpolation.BezierSplineInterpolator;
import me.onesmash.splineinterpolation.Curve;
import me.onesmash.splineinterpolation.SplineCurve;
		...
		for (Point point : points) {
			...
			lastY = (int)pixel.y;
                ...
                if(points.size() >= 2) {
                    ...
                    SplineCurve sc = interpolator.interpolate(xP, yP);
                    Curve[] curves = sc.getCurves();
                    for (int i = 0; i < curves.length; i++) {
                        g.draw(curves[i].getPath());
                        Point2D.Double[] ctrP = curves[i].getControlPoints();
                        g.drawLine((int)ctrP[0].x, (int)ctrP[0].y, (int)ctrP[1].x, (int)ctrP[1].y);
                        g.drawLine((int)ctrP[2].x, (int)ctrP[2].y, (int)ctrP[3].x, (int)ctrP[3].y);
                    }
                }
